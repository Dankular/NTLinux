...
**Owner:** NTLinux
**Status:** Not started (the app/game/driver database itself); its foundational input tool is built — see `ntexports/`
...
against Wine's/ReactOS's own `.spec` files. Built and verified (mechanism
only — see its README for the real-Windows-data gap); the compatibility
