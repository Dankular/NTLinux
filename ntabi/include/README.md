...
`_open_process`, `_open_thread`/`_queue_apc`/`_suspend_thread`/
`_resume_thread`/`_wait_single_alertable`, etc. Not yet consumed by ntdll
(that's real, separate Wine-integration work — see `ROADMAP.md` Phase 2's
"known gap", unchanged by Phase 12); consumed today by
`ntabi/tests/test_ntabi.c`, which exercises the full API for real,
including across real process boundaries and real multi-threaded targets
(60 passing checks as of Phase 12).
